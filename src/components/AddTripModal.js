...
import { addTrip, getTrips, updateTrip } from '../services/TripService';
...
const AddTripModal = (props) => {
  const handleSubmit = (e) => {
    ...
    // Fetch the existing trips
    getTrips()
      .then((response) => {
        const existingTrips = response.data && response.data.length ? response.data : [];

        // Append the new trip
        const updatedTrips = [...existingTrips, newTrip];

        // Update the trips in the API
      ...
      .catch((error) => {
        alert('Failed to fetch existing trips');
      });
  };
